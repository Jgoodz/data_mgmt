...
for entity_col in range(0,len(entity_list)):
	...
	#ORACLE EXAMPLE TO FORMATE DATE: TO_CHAR(START_DATE, 'YYYY/MM/DD HH:MI:SS') AS Load_Start
	for row in c:
...
with open(dest_csv_file,'wb') as outfile:
	...
	#csv_file.writerow(zip(*output_list))
	for row in output_list:
		csv_file.writerow(row)	
	#for i in range(len(entity_list)):
		#csv_file.writerow([x[i] for x in output_list[])
		#file.writelines('\t'.join(str(j) for j in i) + '\n' for i in top_list) - TRY THIS
